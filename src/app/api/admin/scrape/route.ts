...
// Transliteracija ćirilica -> latinica (srpska)
...
// Prevod DE -> SR (latinica) preko besplatnog Google Translate endpoint-a.
async function translateToSerbian(text: string): Promise<string> {
  if (!text || !text.trim()) return text
  const input = text.slice(0, 4500)
  try {
    // sl=auto da radi i za nemački i za engleski izvor
    const url =
      'https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=sr&dt=t&q=' +
    ...
    if (Array.isArray(json) && Array.isArray(json[0])) {
      const translated = json[0]
        ...
        .join('')
      // Prevod stiže na ćirilici -> u latinicu
      return cyrillicToLatin(translated) || cyrillicToLatin(text)
    }
    return text
  } catch {
    return text
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 2b) Pokupi slike PRVENSTVENO iz kontejnera galerije proizvoda (pouzdanije od cele stranice)
    const gallerySelectors = [
      '[class*="gallery" i]', '[class*="product-image" i]', '[class*="product-media" i]',
      '[class*="pdp" i]', '[id*="gallery" i]', '[data-testid*="gallery" i]',
      '[class*="ProductImage" i]', '[class*="thumbnail" i]', '[class*="slider" i]',
      'picture', '[class*="swiper" i]',
    ]
    ...
    if (galleryImages.length > 0) {
      ...
      $('img[srcset], source[srcset]').each((_, el) => {
        ...
        if (best) {
          ...
          if (!blockedWords.some((w) => low.includes(w))) data.images.push(best)
        }
      })
    }
    ...
    // 2e) Domen-specifična pravila (Louis, Polo)
    const host = (() => { try { return new URL(url).hostname } catch { return '' } })()

    if (host.includes('louis.')) {
      // Louis drži slike proizvoda na media CDN-u; pokupi sve product-media URL-ove iz celog HTML-a
      const louisImgs = html.match(/https?:\/\/[^"'\s]*louis[^"'\s]*\/(?:media|images|produkte|product)[^"'\s]*\.(?:jpg|jpeg|png|webp)/gi)
      if (louisImgs) louisImgs.forEach((m) => data.images.push(m.replace(/\\/g, '')))
      // Louis-ove veličine su dugmad sa data atributima
      $('button[data-testid*="size"], [class*="SizeSelector"] button, [class*="variant-select"] option').each((_, el) => {
        const t = $(el).text().trim()
        if (t && t.length <= 5) data.sizes.push(t.toUpperCase())
      })
    }

    if (host.includes('polo-motorrad')) {
      // Polo slike na njihovom CDN-u
      const poloImgs = html.match(/https?:\/\/[^"'\s]*polo[^"'\s]*\.(?:jpg|jpeg|png|webp)/gi)
      if (poloImgs) poloImgs.forEach((m) => data.images.push(m.replace(/\\/g, '')))
    }

    // Ukloni thumbnail/mini varijante ako postoji ista slika u većoj verziji
    data.images = data.images.map((u) =>
      u.replace(/\/(thumb|thumbnail|small|mini|xs|s)\//i, '/').replace(/[?&](w|width|h|height)=\d+/gi, '')
    )
    ...
    data.sizes = Array.from(new Set([...data.sizes, ...extractSizes(sizeText.join(' ') + ' ' + (data.description || ''))])).slice(0, 16)
