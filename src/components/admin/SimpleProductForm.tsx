...
export default function SimpleProductForm() {
  ...
  const save = async () => {
    ...
    try {
      const payload = {
        ...
        variants: sizes.map((s) => ({ type: 'SIZE', value: s, stock: 5 })),
      }
  ...
  const inp =
    'w-full bg-[#17171D] border border-white/10 rounded-lg px-3.5 py-3 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-[#FF4B1F]/50 transition-colors'
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-[#17171D] border border-white/6 rounded-xl p-6 space-y-5">
        {/* LINK + povuci */}
        <div>
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-4 py-3 rounded-lg bg-[#FF4B1F] text-[#0B0B10] text-sm font-medium hover:bg-[#FF5722] transition-colors disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2 whitespace-nowrap"
            >
        ...
        {(scraped || brand || sizes.length > 0 || images.length > 0) && (
          <div className="rounded-xl border border-[#FF4B1F]/20 bg-[#FF4B1F]/5 p-4 space-y-4">
            <p className="text-xs uppercase tracking-wider text-[#FF4B1F] flex items-center gap-1.5">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {images.map((url, i) => (
                    ...
                    >
                      ...
                      {i === 0 && (
                        <span className="absolute top-1 left-1 bg-[#FF4B1F] text-[#0B0B10] text-[9px] px-1.5 py-0.5 rounded">
                          Glavna
                        </span>
                      )}
                      <button
